...
- Without an explicit style, a pack override of the default tooltip sprites (`minecraft:tooltip/background` + `minecraft:tooltip/frame`) still themes every tooltip; animated sprite strips play their full animation (see Animated textures).
...
- Per-element `rotation` entries (any angle about one axis - the modern 1.21.6+ rule; `origin` and `rescale` included) render with vanilla semantics - the geometry decorative medals and tilted props are built from.
...
Animated pack textures (vertical flipbooks with an `animation` mcmeta) always render as the real flipbook - there is no opt-in and no toggle. When the resolved visual uses at least one animated texture, `generate()` returns the GIF form of `GeneratedObject`, with `getFrameDelaysMs()` carrying the per-frame delays (frames legitimately hold for different times - a shiny-strip frames list like `[0..16, {"index": 0, "time": 100}]` produces one long-hold frame). A caller that wants a single still frame takes one from `getAnimationFrames()` itself:
...
- The full vanilla animation model is honored: `frametime` (default 1 tick = 50 ms), frames lists with int and `{index, time}` entries (a per-frame `time` overrides `frametime`), default top-to-bottom frame order when the list is absent, and `width`/`height` frame-size overrides. `interpolate: true` cross-fades like the vanilla client: each keyframe expands into one blended frame per tick, linearly interpolating the previous keyframe toward the next by `subTick / time` (R/G/B mixed with vanilla's truncating math, alpha kept from the current keyframe so opaque animations stay opaque). Because interpolated playback samples every tick, its cycle reaches the shared 120-step / 1200-tick timeline cap sooner and truncates deterministically past it.
...
- Font glyphs never animate: vanilla does not animate font atlas textures. Server-side glyph animation (server-style codepoint cycling) is the caller's concern.
- Renders without any animated texture stay static, returning a `BufferedImage` exactly as before.

### Limitations

Documented edges of the pack renderer, all deliberate:

- **Fonts:** `ttf` providers render as real vector glyphs (Tier 1): the pack's font file is rasterized with plain Java2D at `ppem = round(size * oversample)` (antialiasing off), scaled down by `1 / oversample` to GUI pixels, and placed on the vanilla baseline honoring the provider `shift`. Each pack's font is a private `java.awt.Font` instance (never globally registered), so packs shipping same-named fonts never collide. A missing or unloadable `ttf`, plus every `unihex` and `legacy_unicode` provider, still parses but does not render - those codepoints fall back to the built-in fonts (a warning is logged once per font). `bitmap` providers whose sheet texture is absent from the pack are skipped with a warning instead of failing the whole font - a deliberate deviation from vanilla for packs referencing unbundled vanilla client sheets (a font whose providers all skip resolves as an empty font).
- **TTF fidelity (Tier 2, not built):** pixel fonts authored for integral ppem (e.g. a pixel `hud.ttf` at 28) reproduce exactly; non-pixel outlines render blocky since the downscale is nearest-neighbor with no coverage averaging. The full vanilla `MinecraftGlyphVector` path (glyph-index selection, `setGlyphPosition` overlap, `Area` outline knockout, COLR/CPAL) is out of scope - no real server pack needs it, and vanilla only alpha-composites glyph textures. TTF glyphs also render as themselves under `&k` obfuscation rather than scrambling among equal-width glyphs (that pool is bitmap/space only, to avoid a full-font scan).
...
- **Animation:** animated pack textures always play (see Animated textures), so a visual carrying one renders a GIF rather than a still; `interpolate` cross-fades per tick (which reaches the cap sooner), and shared timelines cap at 120 steps / 1200 ticks.
